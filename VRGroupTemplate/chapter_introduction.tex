...
In 2014, the Neurorobotics team at the Blue Brain Project (BBP \cite{bbp}), project of the Ecole Federale Polytechnique Federale de Lausanne (EPFL), successfully produced a point-neuron model
...
the mouse brain model possible, a large scale of resources with low latency requirements is needed. Today, only modern supercomputers can provide these resources.
The current functionality of NEST generates large neuronal networks on a supercomputer based only random distributions, not yet from experimental data contained in brain atlases.
...
Brain simulations is a interdisciplinary field that links the diverse fields 
...
understand different kinds of diseases like Alzheimer?s and can help to develop novel therapies.
...
\section{Anatomy of the brain}
 ...
 Functionalities of the grey matter can still be assigned to parts of the brain as shown in Figure \ref{Aufbau2}.
  ...
  The cells in each layer have similar cell type, connections to other layers and connections to the thalamus and other parts.
  ...
  % replace ÒfollowingÓ for the actual neuron, do excitatory neurons excite inhibitory neurons or other excitatory? 
...
However the model objects are only stored on one rank.
...
\caption{Illustration how NEST distributes nodes on ranks based on their id.
...
\section{Parallel IO}
Full-scale data-driven simulation require big data. 
...
Using hyperslab selections each rank can access separate parts of a dataset \ref{fig:hdf5hyperslabs:parallel}.
...
In the second chapter will expand on the implementation, which enables the simulation of a full mouse brain model with NEST.
...
Further, debugging and testing of the generated model and its simulation face new requirements.
