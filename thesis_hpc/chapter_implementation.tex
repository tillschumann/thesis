...
\begin{figure}[ht!]
   	\begin{center}
		...
		\emph{syn} contains post-synaptic neuron ids and synapse model parameters.]{%
    	   ...
    	   \end{center}
    	\caption{%
        The diagrams show the data formats with their related datasets and data types.
     }%
   \label{fig:atlas}
   \end{figure}
...
Taking into account that the data-driven simulation is memory limited, the import modules are limited to their memory consumption.
...
Therefore the theoretical memory consumption is analysed to find an appropriate buffer size for the import synapse module.
...
\end{equation}
$N_{block}$ is adaptable. The import synapse module should use around $5\%$ of the total available memory per process.
