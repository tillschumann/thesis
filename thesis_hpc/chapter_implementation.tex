...
The synapses contain besides the parameters the pre-synaptic and post-synaptic neuron id, which the synapses connects.
...
The second data format defines the storage for the synapse parameters and pre-synaptic and post-synaptic neuron ids.
...
The pre-synaptic neuron ids are grouped together in the \emph{neuron} dataset. This is feasible, because
the number of synapses is way bigger than the number of pre-synaptic ($~11,000$ synapses per pre-synaptic neuron).
...
The scripts are developed to fit on a local laptop.
...
It only regenerates the connections for the voxel, if the previous used experiments for this voxel injects more in total than the current
experiment.
For a parallel implementation this dependency has to be avoided.
To overcome the dependency, the best chosen injection per voxel is calculated before hand.
Thus the loop over the voxels moves after the loop over the experiments.
...
After that the algorithm iterates over the \emph{syn} dataset.
