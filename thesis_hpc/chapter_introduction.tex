...
HBP neurorobotics team built a model mouse brain and a model mouse body, integrating them both into a single simulation and providing a simplified but comprehensive model of how the body and the brain interact with each other. It gives the possibility to
...
The search algorithm iterates over all neurons inside the bluish region.
...
For thread parallelism it supports PThreads\cite{nichols1996pthread} and OpenMP\cite{dagum1998openmp}.
...
\begin{figure}[ht!]
   	\begin{center}
        ...
        \hspace{0.5cm}
        \subfigure[Hyperslab selections are used to distribute loaded data on the processes.]{%
            \label{fig:shortColumnInCircuit}
...
In the second chapter the implementation are presented, which enable the simulation of a full mouse brain model with NEST.
The presented algorithm in chapter 1 were developed to create reduced mouse brain models.
...
A IBM Blue Gene Q system is available for debugging and testing.
...
All in all, the work includes: Defining a data format, rewriting the generation programs and extending the neuronal simulator by a
import module.
Further, debugging and testing of the generated model and its simulation tackle new requirements.
The work was done in a close collaboratoin with Csaba Erose and the visualisation teams from the Blue Brain Project and the IT-Center Aachen.
New validation techniques had to be developed and are partly shown in the following chapters.
